...
var REVIEW = {
    getReadData : function(obj, eType)
    {
        if (obj != undefined) {
            ...
            var pNode = obj.siblings('.view_wrapper');
            var pass_check = '&pass_check=F';
        } else {
            var sHref = PARENT.attr('href');
            var pNode = PARENT.siblings('.view_wrapper');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
					...
					var beforStr = data.read['content'];
					var afterStr = deletetag(beforStr, ['br','p']);
					var sCont = afterStr.replaceAll('<p></p>','').replaceAll('<p><br></p>','');
					var nobrImg = deletetag(sImg, ['img']);
					
                    aHtml.push('<div class="view'+ data.read['block_content_class'] +'">');
					//aHtml.push('<div id="ec-ucc-media-box-'+ data.read['no'] +'"></div>');
                    aHtml.push('<div class="attach">'+sImg+'</div>');
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        aHtml.push('<ul class="boardComment" id="commentList_'+data.read['no']+'">');
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                aHtml.push('<span class="grade '+data.use_point+'"><img src="/_idio/img/ico_point_'+data.comment[i]['comment_point_count']+'.png" alt="'+data.comment[i]['comment_point_count']+'점" /></span>');
                                aHtml.push('</div>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('<div class="byteRating" style="display:none;"><p class="rating ' +data.comment_write['use_point']+ '">' +data.comment_write['comment_point']+ '</p><p class="byte ' +data.comment_write['use_comment_size']+ '">/ byte</p></div>');
                ...
                $(pNode).append('<div id="product-review-read'+data.key+'" class="'+ data.read['block_target_class'] +'" '+ data.read['block_data_attr'] +'>'+aHtml.join('')+'</div>');
				
				$('#prdReview .sCont').each(function(){
...
$(function(){	
	...
	//본문 자동 로딩
	$('.xans-product-review .subject > a').each(function(index){
