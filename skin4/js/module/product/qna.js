        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
					...
					function removeAttributes(input, tags) {
						tags.forEach(tag => {
							if (tag === 'img') {
								var regExp = new RegExp("<" + tag + "\\s+([^>]*?)(style=\"[^\"]*\")[^>]*?(src=\"[^\"]*\")[^>]*>", "gi");
								input = input.replace(regExp, function (match, p1, p2, p3) {
									return "<" + tag + " " + p2 + " " + p3 + ">";
								});

								// Handle cases where style attribute might not be present but src is
								regExp = new RegExp("<" + tag + "\\s+([^>]*?)(src=\"[^\"]*\")[^>]*?>", "gi");
								input = input.replace(regExp, function (match, p1, p3) {
									return "<" + tag + " " + p3 + ">";
								});

								// Handle cases where src attribute might not be present but style is
								regExp = new RegExp("<" + tag + "\\s+([^>]*?)(style=\"[^\"]*\")[^>]*?>", "gi");
								input = input.replace(regExp, function (match, p1, p2) {
									return "<" + tag + " " + p2 + ">";
								});

							} else {
								var regExp = new RegExp("<" + tag + "[^>]*>", "gi");
								input = input.replace(regExp, function (match) {
									return "<" + tag + ">";
								});
							}
						});
					...
					var sCont = deletetag(beforStr, ['br', 'p', 'img']);
					sCont = removeAttributes(sCont, ['br', 'p', 'img']);
